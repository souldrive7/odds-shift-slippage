...
| Fig. 1B Santander points; Fig. 2 (main text) Santander points; the Sec. 3 cap sweep, its unweighted capped twins (`unweighted_mds{c}`, the control for the capped weighted arm) and the inversion's over-correction | `santander_capsweep.json` | `$RUN --dataset santander --part capsweep $ARR` for a full recompute; `python code/capsweep_twins.py --dataset santander --arrays-dir <arrays>` adds only the `unweighted_twin` blocks and the `retrain_check` to an existing file (this is how the shipped file was produced on 2026-09-12: the twins were trained on a second machine, where the canonical `unweighted`, `weighted`, `weighted_mds0.7` and `weighted_mds2` arrays were not present; LightGBM did not reproduce those four bit for bit when retrained there, so the canonical entries were kept and only the twins were added. The canonical arrays themselves were trained on the first machine and are archived with the release; `retrain_check` therefore documents run-to-run variation across two machines, not a lost artefact) | 343 s (5.7 min) for the full part; ~4 min for the twins | `\ncap…`, `\nSmds…TEtaC`, `\nSmds…bPriorMax`, `\nSmds…overMedian`, `\nSmds…Nraw`, `\nSmds…NplIsoPrior`; also writes `tab_capsweep_santander_body.tex` (Supplement S7) |
