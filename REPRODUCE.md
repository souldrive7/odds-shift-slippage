...
| Fig. 1B Santander points; Fig. 2 (main text) Santander points; the Sec. 3 cap sweep, its unweighted capped twins (`unweighted_mds{c}`, the control for the capped weighted arm) and the inversion's over-correction | `santander_capsweep.json` | `$RUN --dataset santander --part capsweep $ARR` | 343 s (5.7 min) without the twins | `\ncap…`, `\nSmds…TEtaC`, `\nSmds…bPriorMax`, `\nSmds…overMedian`, `\nSmds…Nraw`, `\nSmds…NplIsoPrior`; also writes `tab_capsweep_santander_body.tex` (Supplement S7) |
